...
<form method="POST" action="{{ route('suspects.store') }}" enctype="multipart/form-data" id="suspect-form">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 p-3 position-sticky top-0 bg-white"
        ...
        <h1 class="mb-0 fw-bold" style="font-size:1.1rem;">إضافة مسجل جديد</h1>
        <div class="d-flex gap-2">
            <a href="{{ route('suspects.index') }}" class="btn btn-brutal-ghost px-4">← إلغاء</a>
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-4">
                        ...
                        <x-select-with-add
                            ...
                            :selected="old('registration_category')"
                            placeholder="— اختر —"
                            label="فئة المسجل"
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <textarea id="distinguishing_marks" name="distinguishing_marks" rows="2"
                                  class="form-control @error('distinguishing_marks') is-invalid @enderror">{{ old('distinguishing_marks') }}</textarea>
                    </div>
...
document.addEventListener('keydown', function(e) {
    ...
    const formElements = Array.from(document.querySelectorAll(
        '#suspect-form input:not([type="hidden"]):not([disabled]), #suspect-form select:not([disabled]), #suspect-form textarea:not([disabled]), #submit-btn'
    ));
